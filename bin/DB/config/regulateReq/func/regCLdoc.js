 ...
 */
module.exports = (docObj, MToptions, payload) => {
    let { CLdoc, is_upd = false} = MToptions;
    now = new Date();
    for (key in CLdoc) {
        if (is_upd) {
            // 在更新的情况下 如果不可更改 则跳过： 比如创建时间 后面的代码就不用执行了
            if (CLdoc[key].IS_fixed) {
                if (docObj[key] !== undefined) return `docRegulate 修改时 不可修改 IS_fixed 为true 的字段 [${key}].`;
            } else {
            ...
            if ((CLdoc[key].required === true) && (docObj[key] === null || docObj[key] === undefined)) {
                return `docRegulate 创建时 必须添加 [docObj.${key}] 字段`;
            }
            let errMsg = followCLobj(CLdoc, docObj, key, payload)
            if(errMsg) return errMsg;
        }
